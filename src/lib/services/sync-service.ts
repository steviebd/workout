...
function buildSyncResponse(
  ...
  workoutExercises: SyncData['workoutExercises'],
  workoutSets: SyncData['workoutSets']
): SyncData {
  return {
    lastSync,
...
export async function syncPull(
  ...
  const db = dbOrTx;
  const sinceParam = lastSyncAt ?? undefined;
  ...
  const [exercisesResult, templatesResult, workoutsResult] = await db.batch([
    db
      ...
      .from(exercisesTable)
      .where(sinceParam
        ? and(eq(exercisesTable.workosId, workosId), gt(exercisesTable.updatedAt, sinceParam))
        : eq(exercisesTable.workosId, workosId))
    ...
    db
      ...
      .from(templatesTable)
      .where(sinceParam
        ? and(eq(templatesTable.workosId, workosId), gt(templatesTable.updatedAt, sinceParam))
        : eq(templatesTable.workosId, workosId))
    ...
    db
      ...
      .from(workoutsTable)
      .where(sinceParam
        ? and(eq(workoutsTable.workosId, workosId), gt(workoutsTable.updatedAt, sinceParam))
        : eq(workoutsTable.workosId, workosId))
  ...
  if (workoutIds.length === 0) {
    return buildSyncResponse(lastSync, exercises, templates, workouts, [], []);
  }
